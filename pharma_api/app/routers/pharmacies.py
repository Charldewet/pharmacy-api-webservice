...
def get_reconciliation_summary(pharmacy_id: int, month: str = Query(..., description="Month in YYYY-MM format")):
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            if not bank_stats or bank_stats['total_lines'] == 0:
            ...
            total_lines = bank_stats['total_lines']
            reconciled_lines = bank_stats['reconciled_lines']
            unmatched_lines = bank_stats['unmatched_lines']
            bank_total = float(bank_stats['bank_total']) if bank_stats['bank_total'] else 0.0
            ...
            ledger_total = float(ledger_result['ledger_total']) if ledger_result and ledger_result['ledger_total'] else 0.0
